...
>
> **REGIME CAVEAT ON THE CAVEAT (2026-07-31, after `a35e3a8`).** 95.6% is a **lifetime**
> rate over a store whose writer changed mid-life, and it is not the current rate. The
> content_hash migration (`9a9fb50`, 2026-07-22) cut copies-per-event from **23.0 → 1.32**;
> the cross-session leak fix (`a35e3a8`, 2026-07-31) closed the remainder within a shard.
> Rows/day fell ~400x at the 07-22 boundary while **new distinct events/day went up**
> (25–383 before, 81–242 after) — so the collapse is dedup, not reduced activity. Use 95.6%
> only for the accumulated corpus; the live writer duplicates at ~1.3 copies/event **within a
> shard**. Cross-*shard* duplication (fleet 93.3%, 14.9 copies, 32% of events in >1 shard) is
> **unfixed** — a per-shard guard cannot see other shards. See
> `forum/cbp-the-duplication-was-fixed-nine-days-ago-and-the-remedy-is-inverted-2026-07-31.md`.
>
...
| 11 | **the dedup guard is a no-op and 95.6% of tier 1 is re-ingested duplicates** | `db.ts:285` (`existsContentHash`), `conversation-capture.ts` via `pre-compact.ts:49` / `session-end.ts:36` | The only guard against re-storing a transcript turn is `SELECT 1 FROM observations WHERE session_id = ? AND content_hash = ?`. **`content_hash` is NULL on 702,058 / 704,049 rows (99.72%)** and `= NULL` is never true in SQL, so the guard cannot match; and **`session_id` is effectively a constant** — one value covers 697,888 rows (99.12%) from 2026-03-15 to 2026-07-31 — so it carries no discrimination either. Every PreCompact and SessionEnd therefore re-walks the transcript and re-inserts every turn still visible in it. Measured: 704,049 rows are **31,219 distinct `(tool_name, input_summary, output_summary)`** (22.6 copies each); Conversation is **689,549 rows / 17,808 distinct turns** (38.7 copies each); the worst single turn appears **414 times across 22 distinct days and 28 cwds**. Consequence at the top of the stack: `tool_transitions` holds **1,576,273** `Conversation→Conversation` edges from 689,549 Conversation rows (2.29x), and the `patterns` upsert accumulates that across runs into **frequency 43,581,138** for the tautology — **the #1 item in 1,212 of 1,217 briefings (defect #7) is authored by this bug**. `scripts/audit_tier1_duplication.py --check`. **PATCHED `a35e3a8` (2026-07-31), and the diagnosis above is half wrong.** (i) The `content_hash`-is-NULL half was fixed by `9a9fb50` on **2026-07-22**, nine days before this row was written: copies-per-event **23.0 → 1.32** at that boundary, while *new distinct events/day rose* (25–383 → 81–242), so it is dedup and not an activity drop. The 95.6% is a lifetime rate spanning two writers. (ii) The `session_id`-is-constant half is **not a duplication cause — it was suppressing duplication.** Inside the regime where the guard can fire (since 07-23): rows under the constant id are **288/288 distinct (1.00 copies)**; rows under *real* session ids are **1,593/1,135 (1.40 copies, 28.8% re-stored)**, with zero same-session guard failures. A constant session degenerates a session-scoped guard to store-global, which is the correct guard — so "fix the session id" alone would have **raised** duplication. The real defect was the session predicate itself, added by `9a9fb50` to protect the tool path, which never calls this guard (`capture()` stores `content_hash` as metadata only). `a35e3a8` drops the predicate, adds `idx_obs_content_hash` (the guard was a full table scan without it) and `observations.scored_by`/`SCORER_VERSION` (keep-first freezes a row's SNARC vector at whichever generation saw it first; 17.1% of distinct turns carry mixed `(surprise, novelty)` across copies today). Gauge: `node scripts/acceptance_dedup_scope.mjs` — 6 checks, pre-fix 3 red / post-fix 6 green, both runs reaching their own end. **Still unfixed: cross-*shard* replay** (fleet 93.3%, 14.9 copies/event, 32% of events in >1 shard, worst 35) — a per-shard guard cannot see other shards. **Still unfixed: `patterns.frequency = 43,581,138`** — the writer stopped on 07-22, the row did not decay, and it remains #1 in 1,212/1,217 briefings; `upsertPattern` only ever adds, so it stays #1 until someone writes to that row (a data migration on a live store = operator decision) |
